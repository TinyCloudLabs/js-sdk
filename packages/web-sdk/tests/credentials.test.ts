...
const HOLDER = "did:key:z6MkActive"; const ORIGIN = "https://witness.credentials.org"; const REQUEST = "RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR";
...
test("storage succeeds only after active-owner readback", async () => {
  ...
  const saved = await storeCredential({ kv: memory.service, verified, requirement: req, requirementDigest: digest, activeOwnerDid: HOLDER, now: new Date("2030-01-01T00:00:01Z") }); expect((await findStoredCredential({ kv: memory.service, requirement: req, holderDid: HOLDER, now: new Date("2030-01-01T00:00:02Z") }))?.recordId).toBe(saved.record.recordId); await expect(storeCredential({ kv: kvMemory(true).service, verified, requirement: req, requirementDigest: digest, activeOwnerDid: HOLDER })).rejects.toMatchObject({ code: "VERIFIED_NOT_SAVED", recoverable: true }); await expect(storeCredential({ kv: memory.service, verified: { ...verified, holderDid: "did:key:other" }, requirement: req, requirementDigest: digest, activeOwnerDid: HOLDER })).rejects.toBeInstanceOf(CredentialError);
});
