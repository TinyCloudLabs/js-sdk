...
function waitForCompletion(popup: Window, origin: string, state: string, requestId: string, timeoutMs: number): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    popup.postMessage({ type: REQUEST_TYPE, version: REQUEST_VERSION, state, requestId }, origin);
  });
}
...
export class CredentialsService {
  ...
  async ensure(options: CredentialsEnsureOptions = {}): Promise<CredentialsEnsureResult> {
    ...
    const requestId = randomOpaqueState();
    ...
    await waitForCompletion(popup, descriptor.issuerOrigin, state, requestId, options.timeoutMs ?? 120_000);
