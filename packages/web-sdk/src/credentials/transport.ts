...
export class OpenCredentialsHttpTransport implements CredentialAcquisitionTransport {
  ...
  async state(id: string, verifier: string, signal?: AbortSignal): Promise<CredentialRequestState> {
    let raw = object(await this.request("state", { id, verifier, signal }), "state response");
    ...
    if (raw.state === "challenge_required") {
      const step = this.proofStep(); const challenge = object(await this.request("challenge", { id, verifier, method: "POST", body: { step, stepVersion: 1 }, signal }), "challenge response");
      if (challenge.type !== "tinycloud.credentials/challenge/v1" || challenge.step !== step || challenge.stepVersion !== 1) throw new CredentialError("REQUEST_SUBSTITUTED", "Credential challenge is substituted");
      this.nonces.set(id, opaque(challenge.challengeNonce, "challengeNonce")); raw = { ...raw, state: "proof_required" };
    }
    ...
    if (raw.state === "proof_required") { const type = this.proofStep(); return { ...base, state: "pending", nextStep: { id: type, type, version: 1, constraints: {} } }; }
