...
export async function findStoredCredential(input: { readonly kv: IKVService; readonly requirement: CredentialRequirement; readonly holderDid: string; readonly ownerDid: string; readonly now?: Date }): Promise<StoredCredentialRecord | undefined> {
  ...
  for (const key of listing.data.keys.sort()) {
    ...
    if (result.ok && stored(result.data.data) && result.data.data.ownerDid === input.ownerDid && credentialMatchesRequirement(input.requirement, result.data.data, input.holderDid, input.now)) return result.data.data;
  }
  return undefined;
}

export async function storeCredential(input: { readonly kv: IKVService; readonly verified: VerifiedCredential; readonly requirement: CredentialRequirement; readonly requirementDigest: string; readonly activeHolderDid: string; readonly activeOwnerDid: string; readonly now?: Date }): Promise<{ readonly record: StoredCredentialRecord; readonly receipt: CredentialStorageReceipt }> {
  if (input.verified.holderDid !== input.activeHolderDid) throw new CredentialError("HOLDER_MISMATCH", "Credential holder does not match the active session key");
  ...
  if (!(await verifyStorageReceipt(readback.data.data, receipt, input.activeOwnerDid, input.activeHolderDid))) throw new CredentialError("VERIFIED_NOT_SAVED", "Credential storage receipt is invalid");
