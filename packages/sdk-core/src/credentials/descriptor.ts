...
function origin(value: unknown, label: string): string { const candidate = text(value, label); let url: URL; try { url = new URL(candidate); } catch { fail(`${label} is invalid`); } if (url!.protocol !== "https:" || url!.origin !== candidate || url!.username || url!.password) fail(`${label} is invalid`); return candidate; }
...
export function validateCredentialFlowDescriptor(value: unknown): CredentialFlowDescriptor {
  ...
  exact(raw, ["type", "contractVersion", "protocol", "profile", "profileVersion", "display", "accessibility", "theme", "issuer", "interaction", "format", "claims", "subjectRelationship", "inputs", "steps", "holderBinding", "endpoints", "lifecycle", "status", "revocation", "presentation"], "descriptor");
  ...
  const issuer = object(raw.issuer, "issuer"); exact(issuer, ["did", "origin", "kid"], "issuer"); origin(issuer.origin, "issuer.origin"); const did = text(issuer.did, "issuer.did", /^did:[a-z0-9]+:.+$/); const kid = text(issuer.kid, "issuer.kid"); if (!kid.startsWith(`${did}#`)) fail("issuer.kid is invalid");
  const interaction = object(raw.interaction, "interaction"); exact(interaction, ["origin", "pathTemplate"], "interaction"); if (origin(interaction.origin, "interaction.origin") !== "https://credentials.org" || interaction.pathTemplate !== "/credentials/acquire/{requestId}") fail("interaction endpoint is invalid");
  ...
  if (!Array.isArray(raw.claims) || raw.claims.length === 0 || raw.claims.length > 64) fail("claims are invalid"); const claimNames = new Set<string>(); for (const value of raw.claims) { const claim = object(value, "claim"); exact(claim, ["name", "matching", "selectiveDisclosure"], "claim"); const name = text(claim.name, "claim.name", CLAIM_NAME); if (claimNames.has(name) || claim.matching !== "normalized_exact" || typeof claim.selectiveDisclosure !== "boolean") fail("claim is invalid"); claimNames.add(name); }
