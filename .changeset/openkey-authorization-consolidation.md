...
- `node-sdk.signInWithOpenKeyResult` now enforces STRICT selectedActionKeys/permissions consistency: `selectedActionKeys` must cover every non-required capability in `signedMessage`, and every returned `permissions` entry action must appear in `signedMessage` (empty `selectedActionKeys` with capability-bearing SIWE is rejected; broader `permissions` entries are rejected).
